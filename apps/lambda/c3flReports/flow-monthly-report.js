...
var DynStream = require('./flow-dyn-stream');
var CSVTransform = require('./flow-transform-stream');
...
function sidRawToCsv(context) {
  ...
  var s3obj = new aws.S3({
    params: {
      Bucket: bucket_name,
      Key: folder_name + '/' + file_dt_tag + '_flow' + batch + '_bklg.zip'
    }
  });
  //
  ...
  s3dev.getObject(params_dev, function(err, data) {
    ...
    sids = Object.keys(dev_state_by_sids);
    //console.log(sids);
    //
  ...
  var count = batch;
  // This function will be called periodically with settimeout.
