...
DynStream.prototype._read = function read() {
  ...
  dynamo.describeTable({ TableName: this._tablename }, function (err, data) {
    if (err) {
      ...
      //var desc = { dt: "Station ID: ", wa: sid, md: '' }
      ...
      var desc = ",,";
      self.push(desc);
      //var title = { dt: 'Time', ra: 'Level', md: 'Status' }
      var title_CSV = "Time,  Level(mRL), Status";
      self.push(title_CSV);
  ...
  dynDoc.query(params, function (err, data) {
    if (err) {
      ...
      for (var idx = 0; idx < data.Items.length; idx++) {
        ...
        var record_csv = dt_local + ',' + record.wa.toString() + ',' + record.md;
        //self.push(record);
